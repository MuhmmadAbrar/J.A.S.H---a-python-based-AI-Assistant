...
while (i<1):
    ...
    try:
        ...
        if ('nearby') or ('open nearby') in message:
            mydb=mysql.connector.connect(host='localhost',database='nearby',user='root',passwd='sample')
            mycursor=mydb.cursor()
            if mydb.is_connected():
                print('Database Connected succesfully')
                print('FOR HOTELS PRESS [1] FOR HOSPITALS PRESS [2]')
                kay=input('What are you searching for ? : ')
                print('This service is available only for ARUMBAKKAM , ANNANAGAR , CHETPET AND VADAPALANI.')
