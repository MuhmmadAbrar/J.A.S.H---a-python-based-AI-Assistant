...
mydb=mysql.connector.connect(host='localhost',database='nearby',user='root',passwd='sample')                
mycursor=mydb.cursor()

mycursor.execute("SELECT * FROM Hospitals where Area='ARUMBAKKAM'")
ar=mycursor.fetchall()

mycursor.execute("SELECT * FROM Hospitals where Area='ANNA NAGAR'")
an=mycursor.fetchall()

mycursor.execute("SELECT * FROM Hospitals where Area='VADAPALANI'")
vd=mycursor.fetchall()

mycursor.execute("SELECT * FROM Hospitals where Area='CHETPET'")
...
while (i<1):
    ...
    try:
        ...
        if 'goodbye' in message or ('power off') in message or ('stop') in message:                          
            ...
            rand ="I'm fine! What I you do for you??"
            speak(audio)
        ...
        if ('nearby') in message:
            ...
            choice=int(input('Enter [1] for HOSPITALS and [2] for RESTAURANTS : '))
